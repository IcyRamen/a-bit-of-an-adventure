...
def levelOne():
    ...
    def main():
        ...
        level = [
            ...
            "P    PPPPPPPP                being a dev   P",
            "P            P       P        really sucks P",
            "P             P            PPPPPPP         P",
            "P              PPPPPPPPP                P  P",
            "P                                      P   P",
            "P         PPPPPPP                   P      P",
    ...
    class Player(Entity):
        ...
        def __init__(self, x, y):
            ...
            self.images = []
            self.images.append(pygame.image.load(os.path.join('.', 'bin', 'YouLessPix_platform.png')).convert_alpha())
            self.images.append(pygame.image.load(os.path.join('.', 'bin', 'sprite_1.png')).convert_alpha())
            self.images.append(pygame.image.load(os.path.join('.', 'bin', 'sprite_2.png')).convert_alpha())
            self.images.append(pygame.image.load(os.path.join('.', 'bin', 'sprite_3.png')).convert_alpha())
            self.images.append(pygame.image.load(os.path.join('.', 'bin', 'sprite_4.png')).convert_alpha())
            self.images.append(pygame.image.load(os.path.join('.', 'bin', 'sprite_5.png')).convert_alpha())
            self.index = 0
            self.image = self.images[self.index]

        def update(self, up, down, left, right, platforms):
            if up:
                # Only jump if on the ground, also do animations
                if self.onGround:
                    self.yVelocity -= 16
